    ...
    **kwargs : Dict
    ...
    def validation_step(self, batch, batch_idx=None, dataloader_idx=None) -> torch.Tensor:
        ...
        if(self.n_beams>0):
            ...
            #     with open("./score_prime.txt",'a') as f:
            #         sum = np.sum(matches)
            #         aaPre = sum / len(pred)
    ...
    def predict_step(
        ...
        peptides , _ = self.forward(batch[0], batch[1], batch[2])
        import os
        
            
        with open("./denovo_meta.txt",'a') as f:
            for i in range(len(peptides)):
                # print("label:",batch[2][i], ":" , peptides[i] , "\n")
                if batch[2][i].replace("L", "I") == "".join(peptides[i]).replace("L","I"):
                    answer_is_correct = "correct"
                else:
                    answer_is_correct = "incorrect"
                    
                f.write("label: " + batch[2][i] + " prediction: " + "".join(peptides[i]) )
