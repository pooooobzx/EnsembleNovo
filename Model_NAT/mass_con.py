...
r'''
extern "C" __global__
void inference(float* prob, int* ans, float* aa_mass, float* dp, float* dpMass ,int* lock,float premass,int length,float grid_size) {
    const int AA_num = 28;
    const float tol = 0.1;
    int w = blockIdx.x;
    int dim = blockDim.x;
...
def knapDecode(prob, preMass):
    ...
    dpLock[0,:] = 1
    inference_kernel((length,),(word_num + 1,),(prob,ans,AAmasses,dpProb,dpMass,dpLock,preMass,length,grid_size))
    ans = ans.get()
    dpMass = dpMass.get()
    # # print(ans[:,4,1:])
